...
def _fetch_spotify_data(**kwargs):
    ...
    playlist_URI1 = "3cEYpjA9oz9GiPac4AsH4n"

    spotify_data = sp.playlist_tracks(playlist_URI1)

    filename = "spotify_raw_" + datetime.now().strftime("%Y%m%d%H%M%S") + ".json"

    ti.xcom_push(key="spotify_filename", value=filename)
    ti.xcom_push(key="spotify_data", value=json.dumps(spotify_data))
...
def _read_data_from_s3(**kwargs):
    ...
    bucket_name = "spotify-etl-project-rakeshs"
    prefix = "raw_data/airflow_to_processed/"
    ...
    print(spotify_data)
    ti.xcom_push(key = "spotify_data", value = spotify_data)
...
store_raw_to_s3 = S3CreateObjectOperator(
    ...
    s3_bucket = "spotify-etl-project-rakeshs",
    s3_key="raw_data/airflow_to_processed/{{ task_instance.xcom_pull(task_ids='fetch_spotify_data', key='spotify_filename') }}",
